...
public abstract class AbstractGradFunction implements GradFunction {
    @Override
    ...
    @Override
    public double findMinP( double[] x, double[]p, double a, double c, double eps) {
        final double phi = (3 - sqrt(5)) / 2;
        double x2, x1, x3, xi = 0, f2, f1, f3, fi, d, e, g, tol;
        boolean accepted;
        x2 = x1 = x3 = a + phi * (c - a);
        f2 = f1 = f3 = findFx(MatrixUtil.add(x, MatrixUtil.multiply(p, x2)));
        d = e = c - a;
        do {
            accepted = false;
            g = e;
            ...
            d = abs(xi - x2);
            fi = findFx(MatrixUtil.add(x, MatrixUtil.multiply(p, xi)));
            if (fi <= f2) {
                if (xi >= x2) {
                    a = x2;
                } else {
