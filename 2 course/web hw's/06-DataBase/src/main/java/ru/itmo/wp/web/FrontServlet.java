...
import ru.itmo.wp.model.exception.ConfirmationException;
...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        List<String> allMethods = Arrays.asList("before", route.getAction(), "after");
        List<Class<?>> cls = Arrays.asList(HttpServletRequest.class, Map.class);
        for (String currentMethod : allMethods) {
            Method method = null;
            ...
            try {
                ...
                } else if (cause instanceof ConfirmationException) {
                    ConfirmationException c = (ConfirmationException) cause;
                    view.put("error", c.getMessage());
                } else {
        ...
        try {
            ...
            throw new ServletException("Can't render template [pageClass=" + pageClass + ", action=" + route.getAction() + "]", e);
        }
    }
