...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        //add Decoder
        File file = new File(root, java.net.URLDecoder.decode(request.getUri(),
                StandardCharsets.UTF_8.name()));
        //add Default file
        if (!file.isFile())
            file = new File(root, java.net.URLDecoder.decode(request.getUri(),
                    StandardCharsets.UTF_8.name()) + "/index.html");
        if (file.isFile()) {
    ...
    private String getEtag(byte[] body) {
        return Integer.toString(Arrays.hashCode(body));
    }
    ...
    private static final class Request {
        ...
        private String getHeader(String name) {
            //add default value (if not contains "If-None-Match" for e.g.)
